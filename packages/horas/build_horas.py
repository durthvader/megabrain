...
Camada 2 da máquina (montador). Consome, por padrão:
  - projects/horas-<regiao>/data/processed/dados.json
  - projects/horas-<regiao>/content/narrativa.json
e gera o PPTX em projects/horas-<regiao>/deliverables/ no padrão Alloha.
...
def main():
    ...
    dados_path = Path(args.dados) if args.dados else sandbox / "data" / "processed" / "dados.json"
    nar_path = Path(args.narrativa) if args.narrativa else sandbox / "content" / "narrativa.json"
    ...
    saida = Path(args.saida) if args.saida else sandbox / "deliverables" / f"Horas_{slug.capitalize()}_Executivo.pptx"
