...
```
pasta da regional (exports .xlsx)          docs/horas/narrativa_<regiao>.json
  └─ subpastas = assuntos                    (textos: títulos, causas, ações)
     (sobreaviso, horaextra, feriados, …)          │
            │                                       │
            ▼                                       ▼
   extrair_horas.py  ──►  dados_<regiao>.json  ──►  build_horas.py  ──►  Horas_<Regiao>_Executivo.pptx
        (números)                                 (números + textos)
```

- **`scripts/horas/extrair_horas.py`** — lê a pasta de exports e normaliza tudo num JSON
  (`outputs/horas/dados_<regiao>.json`). Tolera acento/cp1252, cabeçalho na linha 1 ou 3,
  legenda "Filtros aplicados", número como texto e nomes de arquivo com sufixo "(1)".
- **`docs/horas/narrativa_<regiao>.json`** — só os **textos** (títulos, decisão, causas, ações,
...
- **`scripts/horas/build_horas.py`** — junta JSON + narrativa e gera o `.pptx` no padrão Alloha.
...
```bash
# 1. copie e edite os textos
cp docs/horas/narrativa_ceara.json docs/horas/narrativa_piaui.json
#    (troque regiao, codigo, data_foto e revise títulos/causas/ações)

# 2. extraia os dados da pasta da nova regional
python scripts/horas/extrair_horas.py --pasta "C:/.../hubhoras/pi" --regiao piaui

# 3. monte
python scripts/horas/build_horas.py --regiao piaui
```

As apresentações são **independentes** — cada regional tem seu `dados_*.json`,
`narrativa_*.json` e `.pptx`.
